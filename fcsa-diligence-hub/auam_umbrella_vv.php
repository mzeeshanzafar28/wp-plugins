...
class auam_UmbrellaAccessManagementVV

{
...

    {
        ...
        add_shortcode('umbrella-request-access', array($this, 'umbrella_memberpress'));



        // AJAX action hook

        add_action('wp_ajax_process_requestvv', array($this, 'process_requestvv'));

        add_action('wp_ajax_nopriv_process_requestvv', array($this, 'process_requestvv'));
...
{
    $recruiter_user_ids = $this->get_recruiter_user_ids();
    $output = '';

    $user_id = get_current_user_id();

    if ($this->is_umbrella($user_id)) {
        ...
        foreach ($recruiter_user_ids as $recruiter_user_id) {
            $recruiter_user_info = get_userdata($recruiter_user_id);

            $args = array(
                'author' => $recruiter_user_id,
                'post_type' => 'recruiter-pack',
                'posts_per_page' => -1,
            );
            ...
            foreach ($posts as $post) {
                ...
                $request_status = $this->get_request_status($user_id, $pack_id, $recruiter_user_id);
                ...
                if ($request_status === 'pending') {
                    ...
                    $output .= '<div  class="request-button" style="cursor: pointer;" data-recruiter-id="' . $recruiter_user_id . '" data-pack-id="' . $pack_id . '"><span class="fas fa-plus-circle"></span> Send access request</div>';
                }
                        ...
                        if (packId) {
                            ...
                            $.post(custom_ajax_obj.ajaxurl, data, function (response) {
                                if (response.trim() !== "") {
                                    ...
                                    var requestButton = $(\'[data-recruiter-id="\' + userId + \'"][data-pack-id="\' + packId + \'"]\');
    ...
    function process_requestvv()

    {

        global $wpdb;

        if (isset($_POST["recruiter_id"]) && isset($_POST["post_ids"])) {
            ...
            $link = $this->get_page_or_post_permalink_by_shortcode('[umbrella-request-access]');

            $this->axon_user_notification->insert_notification($recruiter_id, $content, $link);

            $this->email->send_email_request($user_id, $recruiter_id , $pack_ids, "umbrella" );

            $counter = 0; // Initialize a counter

            foreach ($pack_ids as $pack_id) {
                // Check if a request with the same user_id, recruiter_id, and pack_id and status != 'refused' already exists
                ...
                $existing_request = $wpdb->get_var(
                    ...
                    $wpdb->prepare(
                        ...
                        "SELECT COUNT(*) FROM $table WHERE user_id = %d AND recruiter_id = %d AND pack_id = %d AND status != %s",
